...
def get_address(i: int, j: int, interleave_handler: InterleaveHandler, base_addresses: List[int], element_size_in_bytes: int):
    ...
    base_address = base_addresses[channel_id]
...
def hbm_to_np_impl(array: dace.data.Array, interleave_handler: InterleaveHandler, base_address: int, element_size_in_bytes: int):
    M, K = array.shape
    for i in range(M):
        for j in range(K):
            address = get_address(i, j, interleave_handler, base_address, element_size_in_bytes)

    return True

def hbm_to_np(interleave_handler: InterleaveHandler, base_addresses: List[int]):
    base_addresses = list()
    ch0_base_address = int(HBM_ADDR_BASE)
    ch_address_space = int(HBM_ADDR_SPACE)
    num_channels = interleave_handler.num_channels

    for ch_id in range(num_channels):
        base_addresses.append(ch0_base_address + ch_id * ch_address_space)

    element_dtype = ARRAY_TO_DTYPE.get(interleave_handler.name, DACE_INPUT_TYPE)

    hbm_to_np_impl(interleave_handler.array, interleave_handler, base_addresses, element_dtype.dtype.bytes)
