...
def three(name, vector_size: int, frag_size: int):
    sdfg = dace.SDFG(name)
    ...
    for i, state in enumerate([wstate, mstate]):
        ...
        #state.add_edge(glb_to_vecin, None, libnode, None, dace.memlet.Memlet(f"frag{i}_A[0:{frag_size}]"))
...
def warmup():
    sdfg = three(name="warmup", vector_size=8192, frag_size=32)
    ...
    for frag_power in range(7, 16, 1):
        ...
        for i in range(N):
            try:
                ...
                sdfg = three(name=f"test_{j}", vector_size=vector_size, frag_size=frag_size)
                sdfg(A=A, B=B, C=C)
