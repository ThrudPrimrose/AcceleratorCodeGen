...
def gemm(A: dace.float32[M, K] @ dace.dtypes.StorageType.Ascend_Global,
         B: dace.float32[K, N] @ dace.dtypes.StorageType.Ascend_Global,
         C: dace.float32[M, N] @ dace.dtypes.StorageType.Ascend_Global):
    for i, j in dace.map[0:2, 0:M, 0:N] @ dace.dtypes.ScheduleType.Ascend_Device:
        tmp = 0
        for k in dace.map[0:K//2] @ dace.dtypes.ScheduleType.Sequential:
            tmp = tmp + A[i, k] * B[k, j]
        C[i, j] = C[i, j] + tmp
...
# Memory locations, and possible paths for GPU
graph = {
    glb: { glb: 0, shared: 1, register: 1 },
    shared: { glb: 1, shared: 0, register: 1 },
    register: { glb: 1, shared: 1, register: 0 },
}
...
...
InsertExplicitMemoryMovement(movement_graph=graph,
                             ... # More parameters
                             ).apply_to(sdfg=sdfg, ...)
